\
// /api/simulator-chat.js — OpenRouter (rollback stable)
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") return json(res, 200, { status: "ok", endpoint: "simulator-chat", llm: "openrouter", model: OPENROUTER_MODEL });
  if (req.method !== "POST") return json(res, 405, { error: "Method not allowed" });

  try {
    if (!OPENROUTER_API_KEY) return json(res, 500, { error: "Missing OPENROUTER_API_KEY" });
    ...
    if (!resp.ok) {
      return json(res, resp.status, { error: "OpenRouter error", details: data });
    }

    const bot_message_raw = data?.choices?.[0]?.message?.content || "Je vous ai bien lu.";
    const bot_message = cleanBot(bot_message_raw);

    // 3) log bot
    await insertMessage(session_id, "bot", bot_message);

    return json(res, 200, { bot_message, humeur: 0, meta: { provider: "openrouter", model: OPENROUTER_MODEL } });
  } catch (e) {
